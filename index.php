...
class Restful
{
     ...
     * @var array
     */
    private $_allowResouces = ['users', 'articles'];

    /**
     * 允许请求的方法
     * @var array
     */
    private $_allowRequestMethods = ['GET', 'POST', 'PUT', 'OPTIONS', 'DELETE'];
     ...
     * @return [type]
     */
    public function _setupResouce()
    {
        $path               = $_SERVER['PATH_INFO'];
        $params             = explode('/', $path);
        $this->_resouceName = $params[1];
        if (!in_array($this->_resouceName, $this->_allowResouces)) {
            throw new Exception('请求资源不被允许访问', 400);
        }
        if (!empty($params[2])) {
            $this->_id = $params[2];
        }
    ...
    private function _handleArticleEdit()
    {
        try {
            ...
            if ($article['user_id'] != $user['user_id']) {
                throw new Exception('你无权编辑此篇文章', 403);
            }
            $body    = $this->_getBodyParams();
    ...
    private function _handleArticleDelete()
    {
        ...
        try {
            $article = $this->_article->view($this->_id);
            if ($article['user_id'] != $user['user_id']) {
                throw new Exception('你无权删除此篇文章', 403);
            }
    ...
    /**
     * 用户登录
     * @param  [type] $PHP_AUTH_USER
     * @param  [type] $PHP_AUTH_PW
     * @return [type]
     */
