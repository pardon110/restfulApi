...
class User
{
    ...
    {
        if (empty($username)) {
            throw new Exception("用户不能为空", ErrorCode::USERNAME_CANNOT_EMPTY);
        }
        if (empty($password)) {
    ...
    public function register($username, $password)
    {
        ...
        // 写入数据库
        $sql = 'INSERT INTO `user`(`username`,`password`,`created_at`) VALUES (:username,:password
        ,:created_at)';
     ...
     * @return string
     */
    private function _md5($string, $key = 'pardon')
    {
